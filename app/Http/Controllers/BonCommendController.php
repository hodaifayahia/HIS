...
class BonCommendController extends Controller
{
    ...
    // app/Http/Controllers/BonCommendController.php
...
public function downloadPdf(Request $request, $id)
    {
        try {
            ...
            $bon = BonCommend::with([
                ...
                'creator',
                'items.product.inventorieswithPharmacieStock',
            ...
            // Render the chosen Blade template to HTML
            $html = view("bonCommend.templates.{$template}", [
                'order' => $bon,
                'pharmacyStockMap' => $pharmacyStockMap,
                'pharmacyInventoryDetailsMap' => $pharmacyInventoryDetailsMap ?? [],
            ])->render();

            // Create Browsershot instance with the HTML
            $browsershot = $this->makeBrowsershotInstance($html);
            
            // Configure PDF settings based on template
            if ($template === 'pch') {
                // PCH template specific settings
                $browsershot->paperSize(210, 297, 'mm')  // A4 size
                            ->margins(10, 10, 10, 10)
                            ->showBackground()
                            ->landscape(false);
            } else {
            ...
            // Add additional PDF settings
            $browsershot->setOption('printBackground', true)
                       ->waitUntilNetworkIdle()
                       ->setDelay(500); // Small delay to ensure rendering
            ...
            Log::error('PDF generation failed for bon commend: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => 'Failed to generate PDF. Please check server configuration.'
            ], 500);
        } catch (\Exception $e) {
    ...
    /**
     * Alternative method to save PDF to storage
     */
    public function generateAndSavePdf(Request $request, $id)
    {
        try {
            ...
            $this->configurePdfSettings($browsershot, $template);
    ...
    /**
     * Create Browsershot instance with common configuration
     */
    protected function makeBrowsershotInstance($html)
    {
        $browsershot = Browsershot::html($html)
            ->setOption('args', $this->chromeLaunchArguments());
        
        // Common configuration for all PDFs
        $browsershot->noSandbox() // Required in many server environments
                   ->disableJavascript(false)
                   ->timeout(60) // 60 seconds timeout
                   ->waitUntilNetworkIdle();

        // Set Chrome/Chromium path if resolvable (checks common locations and BROWSERSHOT_CHROME_PATH)
        if ($chromePath = $this->resolveChromePath()) {
            $browsershot->setChromePath($chromePath);
        }

        // Optionally set Node/NPM if needed (uncomment and adjust paths)
    ...
    /**
     * Configure PDF settings based on template type
     */
    protected function configurePdfSettings($browsershot, $template)
    {
        switch ($template) {
            case 'pch':
                // PCH template specific settings
                $browsershot->paperSize(210, 297, 'mm')  // A4
                           ->margins(10, 10, 10, 10)
                           ->showBackground()
                           ->landscape(false)
        ...
        $browsershot->setOption('printBackground', true)
                   ...
                   ->setDelay(500); // Small delay to ensure full rendering
