...
class BonReceptionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Apply filters based on what columns actually exist
        if ($request->status && $request->status !== 'all') {
            // If filtering by is_confirmed status
            if (in_array($request->status, ['completed', 'confirmed'])) {
                $query->where('is_confirmed', true);
            } elseif (in_array($request->status, ['pending'])) {
    ...
    public function getStats()
    {
        try {
            $stats = [
                ...
                'pending' => BonReception::where('is_confirmed', false)->count(),
                'completed' => BonReception::where('is_confirmed', true)->count(),
                'canceled' => 0, // If status column doesn't exist, set to 0
                'rejected' => 0, // If status column doesn't exist, set to 0
    ...
    /**
     * Get dropdown of confirmed bon commends available for reception
     */
    public function getBonCommends()
    {
        try {
            // Get all confirmed bon commends
            // If there's no direct foreign key linking, we'll just return confirmed orders
            $bonCommends = BonCommend::with('fournisseur:id,company_name')
                ...
                ->map(function ($bc) {
                    return [
                        'id' => $bc->id,
                        'bonCommendCode' => $bc->bonCommendCode,
                        'fournisseur_id' => $bc->fournisseur_id,
                        'supplier_name' => $bc->fournisseur?->company_name ?? 'Unknown',
                        'status' => $bc->status,
