...
class WaitListController extends Controller
{
    ...
    public function store(StoreWaitlistRequest $request)
    {
        ...
        if ($existingWaitlist) {
            ...
            return new WaitListResource($existingWaitlist->load(['doctor.user', 'patient', 'specialization']));
        }

        // Create a new waitlist entry if it doesn't exist
        $waitlist = WaitList::create($validatedData);

        return new WaitListResource($waitlist->load(['doctor.user', 'patient', 'specialization']));
    }
    ...
    public function AddPaitentToAppointments(Request $request)
    {
        ...
        // Check if appointmentId is provided and exists
