<template>
  <div class="tw-min-h-screen tw-bg-gradient-to-br tw-from-gray-50 tw-to-gray-100 tw-p-4 lg:tw-p-6">
    ...
    <form @submit.prevent="handleSubmit" class="tw-space-y-6">
      ...
      <Card v-if="formData.items.length > 0" class="tw-border-0 tw-shadow-xl tw-bg-gradient-to-r tw-from-yellow-50 tw-to-orange-50">
        <template #title>
        ...
        <template #content>
          ...
          <div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-4 tw-gap-4 tw-mb-6">
            <div class="tw-bg-white tw-rounded-lg tw-p-4 tw-shadow-sm tw-border tw-border-orange-200">
              <div class="tw-text-sm tw-text-gray-600">Total Amount</div>
              <div class="tw-text-2xl tw-font-bold tw-text-gray-900">{{ formatCurrency(totalAmount) }}</div>
            </div>

            <div class="tw-bg-white tw-rounded-lg tw-p-4 tw-shadow-sm tw-border tw-border-orange-200">
              <div class="tw-text-sm tw-text-gray-600">Items Count</div>
              <div class="tw-text-2xl tw-font-bold tw-text-gray-900">{{ formData.items.length }}</div>
            </div>

            <div class="tw-bg-white tw-rounded-lg tw-p-4 tw-shadow-sm tw-border tw-border-orange-200">
              <div class="tw-text-sm tw-text-gray-600">Status</div>
              <div class="tw-flex tw-items-center tw-gap-2 tw-mt-1">
                <i :class="approvalInfo.needsApproval ? 'pi pi-exclamation-triangle tw-text-yellow-600' : 'pi pi-check-circle tw-text-green-600'"></i>
                <span :class="approvalInfo.needsApproval ? 'tw-text-yellow-800' : 'tw-text-green-800'" class="tw-font-bold">
                  {{ approvalInfo.needsApproval ? 'Approval Required' : 'Direct Confirmation' }}
                </span>
              </div>
            </div>

            <div class="tw-bg-white tw-rounded-lg tw-p-4 tw-shadow-sm tw-border tw-border-orange-200">
              <div class="tw-text-sm tw-text-gray-600">Approver</div>
              <div class="tw-text-lg tw-font-bold tw-text-gray-900">
                {{ approvalInfo.approver?.name || 'N/A' }}
              </div>
            </div>
          </div>
      ...
      <Card class="tw-border-0 tw-shadow-xl">
        ...
        <template #content>
          ...
          <!-- Summary Footer -->
          <div v-if="formData.items.length > 0" class="tw-mt-6 tw-p-4 tw-bg-gray-50 tw-rounded-lg">
            <div class="tw-flex tw-justify-between tw-items-center">
...
const addBulkProducts = () => {
  selectedProducts.value.forEach(product => {
    formData.items.push({
      ...
      unit: bulkDefaults.unit || product.unit || 'pieces',
      price: bulkDefaults.price || 0,
      notes: ''
    })
  })
