  ...
  <div class="tw-min-h-screen tw-bg-gradient-to-br tw-from-slate-50 tw-to-slate-100 tw-p-4 lg:tw-p-6">
              ...
              <Column field="product_id" header="Product" :sortable="true" style="min-width: 250px">
                <template #body="slotProps">
                  <div class="tw-flex tw-items-center tw-gap-3">
                    <Avatar 
                      :label="getProductName(slotProps.data.product_id)?.charAt(0)" 
                      class="tw-bg-teal-100 tw-text-teal-700"
                      shape="square"
                    />
                    <div class="tw-flex-1">
                      <div class="tw-font-semibold">{{ getProductName(slotProps.data.product_id) || 'Select Product' }}</div>
                      <div class="tw-text-xs tw-text-gray-500">
                        Code: {{ getProductCode(slotProps.data.product_id) }}
                      </div>
                    </div>
                    <Button
    ...
    >
      <div class="tw-space-y-4">
        ...
        <!-- Products List -->
        <DataTable 
          v-model:selection="selectedProductsToAdd"
          :value="modalFilteredProducts"
          :paginator="true"
          :rows="10"
        ...
        >
          ...
          <Column field="product_code" header="Code" style="width: 120px">
            <template #body="slotProps">
      ...
      <template #footer>
        <div class="tw-flex tw-justify-between tw-items-center">
          <span class="tw-text-sm tw-text-gray-500">
            {{ selectedProductsToAdd.length }} product(s) selected
          </span>
          <div class="tw-flex tw-gap-2">
            <Button 
              label="Cancel" 
...
const fetchProducts = async () => {
  try {
    ...
    if (response?.data?.data?.data) products.value = response.data.data.data
    else if (response?.data?.data) products.value = response.data.data
    else if (response?.data) products.value = response.data
    else products.value = []
  } catch (err) {
...
const submitForm = async (shouldValidate = false) => {
  try {
    ...
    const response = await axios.post('/api/bon-entrees', payload)
    ...
    if (response.data.status === 'success') {
      ...
      if (shouldValidate) {
        try {
          const validateResponse = await axios.patch(`/api/bon-entrees/${bonEntreeId}/validate`)
...
:deep(.p-datatable .p-datatable-tbody > tr) {
  @apply hover:tw-bg-teal-50 tw-transition-colors;
}
...
:deep(.p-tabview-nav-link) {
  @apply hover:tw-bg-teal-50;
}

:deep(.p-tabview-nav-link.p-highlight) {
  @apply tw-bg-white tw-border-teal-600;
}
...
:deep(.p-steps .p-steps-item.p-highlight .p-steps-number) {
  @apply tw-bg-teal-600 tw-text-white;
}
