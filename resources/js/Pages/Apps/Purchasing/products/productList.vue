  ...
  methods: {
    async fetchProducts(page = 1) {
      ...
      try {
        ...
        // Use the unified purchasing endpoint that fetches from both tables
        const response = await axios.get('/api/purchasing/products', { params });
        if (response.data.success) {
          this.products = response.data.data;
          this.currentPage = response.data.meta ? response.data.meta.current_page : page;
          this.lastPage = response.data.meta ? response.data.meta.last_page : 1;
          this.total = response.data.meta ? response.data.meta.total : response.data.count;
    ...
    async deleteProduct(product) {
      try {
        const source = product.source || 'stock'; // Get source from product
        const response = await axios.delete(
          `/api/purchasing/products/${product.id}?source=${source}`
        );
        if (response.data.success) {
    ...
    async addProduct() {
      ...
      try {
        // Prepare the product data with is_clinical flag
        const productData = {
          ...this.newProduct,
          is_clinical: this.newProduct.is_clinical || false // Ensure boolean value
        };

        const response = await axios.post('/api/purchasing/products', productData);
        if (response.data.success) {
          this.toast.add({
            severity: 'success',
            summary: 'Success',
    ...
    async updateProduct() {
      ...
      try {
        // Prepare the product data with source
        const productData = {
          ...this.editingProduct,
          source: this.editingProduct.source || 'stock' // Use source from product data
        };

        const response = await axios.put(
          `/api/purchasing/products/${this.editingProduct.id}?source=${productData.source}`, 
          productData
        );
        if (response.data.success) {
          this.toast.add({
