      ...
      >
        ...
        <Column field="credit_note_received" header="Credit Note" class="tw-min-w-32">
          <template #body="slotProps">
            <div class="tw-flex tw-items-center tw-gap-2">
              <i 
                :class="slotProps.data.credit_note_received ? 'pi pi-check-circle tw-text-green-500' : 'pi pi-times-circle tw-text-gray-400'"
              ></i>
              <span v-if="slotProps.data.credit_note_number" class="tw-text-xs tw-font-mono">
...
const downloadPdf = async (retour) => {
  try {
    window.open(`/api/bon-retours/${retour.id}/pdf?download=true`, '_blank')
  } catch (err) {
