...
export default {
  ...
  methods: {
    async fetchStockages(page = 1) {
      ...
      try {
        ...
        const response = await axios.get('/api/stockages', { params });
  ...
  // fetchManagers removed: managers are no longer used for stockages
    ...
    viewStockage(stockage) {
      this.$router.push({ name: 'stock.stockages.detail', params: { id: stockage.id } });
    },
    viewStock(stockage) {
  ...
  this.fetchServices();
    ...
    async deleteStockage(stockage) {
      ...
      try {
        const response = await axios.delete(`/api/stockages/${stockage.id}`);
        if (response.data.success) {
  ...
  this.fetchServices();
    ...
    async addStockage() {
      ...
      try {
        const response = await axios.post('/api/stockages', this.newStockage);
        if (response.data.success) {
          this.submitSuccess = true;
          // Refresh the current page to get updated data
          await this.fetchStockages(this.currentPage);
    ...
    async updateStockage() {
      ...
      try {
        const response = await axios.put(`/api/stockages/${this.editingStockage.id}`, this.editingStockage);
        if (response.data.success) {
          this.submitSuccess = true;
          // Refresh the current page to get updated data
          await this.fetchStockages(this.currentPage);
