        ...
        <div class="tw-flex tw-flex-wrap tw-gap-3">
          <Button
            ...
            :class="[
              ...
              activeTab === 'all'
                ? 'tw-bg-blue-600 tw-text-white tw-shadow-lg'
          ...
          <Button
            ...
            :class="[
              'tw-rounded-xl tw-px-6 tw-py-3 tw-font-medium tw-transition-all tw-duration-300',
              activeTab === 'supplies'
                ? 'tw-bg-green-600 tw-text-white tw-shadow-lg'
                : 'tw-bg-gray-100 tw-text-gray-700 hover:tw-bg-gray-200'
            ]"
          ...
          <Button
            ...
            :class="[
              'tw-rounded-xl tw-px-6 tw-py-3 tw-font-medium tw-transition-all tw-duration-300',
              activeTab === 'others'
                ? 'tw-bg-yellow-600 tw-text-white tw-shadow-lg'
                : 'tw-bg-gray-100 tw-text-gray-700 hover:tw-bg-gray-200'
            ]"
...
export default {
  ...
  computed: {
    medicationCount() {
      return this.products.filter(product => product.is_clinical).length;
    },
    suppliesCount() {
      return this.products.filter(product => !product.is_clinical && product.category !== 'Others').length;
    },
    othersCount() {
      return this.products.filter(product => product.category === 'Others').length;
    },
    lowStockAlertCount() {
  ...
  methods: {
    ...
    clearAlertFilters() {
      this.alertFilters = [];
      this.activeTab = 'all'; // Reset to show all products
    ...
    filterProducts() {
      ...
      // Filter by tab
      if (this.activeTab === 'medication') {
        filtered = filtered.filter(product => product.is_clinical);
      } else if (this.activeTab === 'supplies') {
        filtered = filtered.filter(product => !product.is_clinical && product.category !== 'Others');
      } else if (this.activeTab === 'others') {
        filtered = filtered.filter(product => product.category === 'Others');
      }
    ...
    viewStockDetails(product) {
      ...
      this.$router.push({ name: 'stock.products.details', params: { id: product.id } });
    },
    async fetchProductDetails(productId) {
      try {
        const response = await axios.get(`/api/stock/products/${productId}/details`);
        if (response.data.success) {
    ...
    async updateProduct() {
      ...
      try {
        const response = await axios.put(`/api/products/${this.editingProduct.id}`, this.editingProduct);
        if (response.data.success) {
          this.toast.add({
            severity: 'success',
            summary: 'Success',
