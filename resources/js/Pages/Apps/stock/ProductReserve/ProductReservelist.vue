...
const filteredReserves = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(reserve => {
      const productName = reserve.product?.name?.toLowerCase() || ''
      ...
      return productName.includes(query) || 
             pharmacyProductName.includes(query) || 
...
// Load products and stockages
const loadProducts = async () => {
  try {
    // Load regular products
    const productsResponse = await axios.get('/api/purchasing/products')
    const allProducts = productsResponse.data.data || []
    
    // Separate products and pharmacy products
    products.value = allProducts.filter(p => !p.is_pharmacy)
    pharmacyProducts.value = allProducts.filter(p => p.is_pharmacy)
    
    console.log('Products loaded:', products.value.length, 'Pharmacy:', pharmacyProducts.value.length)
  } catch (error) {
...
const loadStockages = async () => {
  try {
    const [stockageResponse, pharmacyStockageResponse] = await Promise.all([
      axios.get('/api/stockages').catch(() => ({ data: { data: [] } })),
      axios.get('/api/pharmacy-stockages').catch(() => ({ data: { data: [] } }))
    ])
    
    stockages.value = stockageResponse.data.data || []
    pharmacyStockages.value = pharmacyStockageResponse.data.data || []
    
    console.log('Stockages loaded:', stockages.value.length, 'Pharmacy:', pharmacyStockages.value.length)
  } catch (error) {
...
const openEditDialog = (reserve) => {
  ...
  form.value = {
    ...
    stockage_id: reserve.stockage_id,
...
const saveReserve = async () => {
  if (!form.value.product_id && !form.value.pharmacy_product_id) {
    toast.add({
      severity: 'warn',
  ...
  try {
    if (dialogMode.value === 'create') {
      ...
      toast.add({
        severity: 'success',
        summary: 'Success',
        detail: 'Product reserved successfully',
        life: 3000
      })
    } else {
...
<template>
  <div class="tw-min-h-screen tw-bg-gradient-to-br tw-from-gray-50 tw-to-gray-100 tw-p-4 sm:tw-p-6 lg:tw-p-8">
    ...
    <div class="tw-mb-8">
      <div class="tw-flex tw-flex-col sm:tw-flex-row sm:tw-items-center sm:tw-justify-between tw-gap-4">
        <div class="tw-flex tw-items-center tw-gap-4">
          <div class="tw-w-16 tw-h-16 tw-bg-gradient-to-br tw-from-indigo-500 tw-to-purple-600 tw-rounded-2xl tw-flex tw-items-center tw-justify-center tw-shadow-lg">
            <i class="pi pi-bookmark tw-text-3xl tw-text-white"></i>
          </div>
          <div>
            <h1 class="tw-text-3xl tw-font-bold tw-text-gray-900">Product Reservations</h1>
            <p class="tw-text-gray-600 tw-mt-1">Manage and track product reservations</p>
          </div>
        </div>
        
        <div class="tw-flex tw-gap-3 tw-flex-wrap">
          <Chip :label="`${totalRecords} Total`" class="!tw-bg-indigo-100 !tw-text-indigo-800 tw-font-semibold" />
          <Chip 
    ...
    <Card class="tw-shadow-2xl tw-rounded-3xl tw-border-0 tw-overflow-hidden">
      <template #content>
          ...
          <div class="tw-flex tw-flex-col lg:tw-flex-row tw-gap-4 tw-items-start lg:tw-items-center tw-justify-between">
            <div class="tw-flex tw-flex-col sm:tw-flex-row tw-gap-3 tw-flex-1 tw-w-full">
              <span class="p-input-icon-left tw-flex-1">
                ...
                <InputText 
                  ...
                  class="tw-w-full tw-rounded-xl !tw-pl-10 !tw-py-3 tw-border-gray-300 focus:tw-ring-2 focus:tw-ring-indigo-500"
                />
              </span>
            ...
            <div class="tw-flex tw-gap-3">
              ...
              <Button 
                ...
                class="tw-bg-gradient-to-r tw-from-indigo-600 tw-to-purple-600 tw-border-0 tw-rounded-xl tw-shadow-lg hover:tw-shadow-xl tw-transition-all"
        ...
        >
          ...
          <Column field="product" header="Product" sortable class="tw-min-w-[250px]">
            <template #body="{ data }">
              <div class="tw-flex tw-flex-col tw-gap-1">
                <span class="tw-font-semibold tw-text-gray-900">
                  {{ data.product?.name || data.pharmacy_product?.name || 'N/A' }}
                </span>
                <span class="tw-text-xs tw-text-gray-500">
                  {{ data.product ? 'Stock Product' : 'Pharmacy Product' }}
                </span>
              </div>
            </template>
          </Column>
          ...
          <Column field="reserver" header="Reserved By" sortable>
            <template #body="{ data }">
              <div class="tw-flex tw-items-center tw-gap-2">
                <div class="tw-w-8 tw-h-8 tw-rounded-full tw-bg-gradient-to-br tw-from-indigo-400 tw-to-purple-500 tw-flex tw-items-center tw-justify-center tw-text-white tw-font-bold tw-text-sm">
            ...
            <template #body="{ data }">
              <div class="tw-flex tw-gap-1 tw-items-center tw-justify-center">
                ...
                <Button 
                  ...
                  icon="pi pi-pencil" 
                  class="p-button-rounded p-button-text p-button-sm !tw-text-indigo-600 hover:!tw-bg-indigo-50"
                  v-tooltip.top="'Edit'"
            ...
            <div class="tw-py-16 tw-text-center">
              <div class="tw-flex tw-justify-center tw-mb-6">
                <div class="tw-w-24 tw-h-24 tw-bg-gray-100 tw-rounded-full tw-flex tw-items-center tw-justify-center">
                  <i class="pi pi-bookmark tw-text-6xl tw-text-gray-300"></i>
                </div>
              </div>
              <h3 class="tw-text-2xl tw-font-bold tw-text-gray-800 tw-mb-2">No Reservations Found</h3>
              <p class="tw-text-gray-600 tw-mb-6">Start by creating your first product reservation</p>
              <Button 
                icon="pi pi-plus" 
                label="Create Reservation" 
                class="tw-bg-gradient-to-r tw-from-indigo-600 tw-to-purple-600 tw-border-0 tw-rounded-xl"
                @click="openCreateDialog"
              />
            </div>
          </template>
    ...
    <Dialog 
      ...
      :header="dialogMode === 'create' ? 'New Reservation' : dialogMode === 'edit' ? 'Edit Reservation' : 'View Reservation'"
      :modal="true"
      :closable="true"
      class="tw-w-full tw-max-w-2xl"
    >
      <div class="tw-space-y-6 tw-p-2">
        <!-- Product Selection (Show only if no pharmacy product selected) -->
        <div v-if="!form.pharmacy_product_id">
          <label class="tw-block tw-text-sm tw-font-semibold tw-text-gray-700 tw-mb-2">
            <i class="pi pi-box tw-mr-2 tw-text-indigo-600"></i>Stock Product *
          </label>
          <Dropdown 
            ...
            optionLabel="name"
            optionValue="id"
            placeholder="Select a stock product"
            filter
            :disabled="dialogMode === 'view'"
            class="tw-w-full"
            @change="() => { form.pharmacy_product_id = null; form.pharmacy_stockage_id = null; }"
          >
            <template #option="{ option }">
              <div class="tw-flex tw-items-center tw-gap-3">
                <div class="tw-w-10 tw-h-10 tw-bg-indigo-100 tw-rounded-lg tw-flex tw-items-center tw-justify-center">
        ...
        <!-- Pharmacy Product Selection (Show only if no regular product selected) -->
        <div v-if="!form.product_id">
          ...
          <Dropdown 
            ...
            :disabled="dialogMode === 'view'"
            class="tw-w-full"
            @change="() => { form.product_id = null; form.stockage_id = null; }"
          >
            <template #option="{ option }">
              <div class="tw-flex tw-items-center tw-gap-3">
        ...
        <!-- Stockage Selection (Only show if product is selected) -->
        <div v-if="form.product_id">
          <label class="tw-block tw-text-sm tw-font-semibold tw-text-gray-700 tw-mb-2">
            <i class="pi pi-warehouse tw-mr-2 tw-text-blue-600"></i>Storage Location
          </label>
          <Dropdown 
            v-model="form.stockage_id"
            :options="stockages"
            optionLabel="name"
            optionValue="id"
            ...
            <template #option="{ option }">
              <div class="tw-flex tw-items-center tw-gap-3">
                <div class="tw-w-10 tw-h-10 tw-bg-blue-100 tw-rounded-lg tw-flex tw-items-center tw-justify-center">
                  <i class="pi pi-warehouse tw-text-blue-600"></i>
                </div>
                <div>
                  <div class="tw-font-semibold">{{ option.name }}</div>
                  <div class="tw-text-xs tw-text-gray-500">{{ option.location || 'No location' }}</div>
                </div>
              </div>
            </template>
          </Dropdown>
        ...
        <!-- Pharmacy Stockage Selection (Only show if pharmacy product is selected) -->
        <div v-if="form.pharmacy_product_id">
          <label class="tw-block tw-text-sm tw-font-semibold tw-text-gray-700 tw-mb-2">
            <i class="pi pi-home tw-mr-2 tw-text-green-600"></i>Pharmacy Storage
          </label>
          <Dropdown 
            ...
            optionLabel="name"
            optionValue="id"
            placeholder="Select a pharmacy storage"
            filter
            :disabled="dialogMode === 'view'"
            class="tw-w-full"
          >
        ...
        <!-- Switch Button to toggle between product types -->
        <div v-if="form.product_id || form.pharmacy_product_id" class="tw-flex tw-justify-center">
          <Button 
            :label="form.product_id ? 'Switch to Pharmacy Product' : 'Switch to Stock Product'"
            icon="pi pi-sync"
            class="p-button-text p-button-sm tw-text-indigo-600"
            @click="() => { 
              if (form.product_id) {
                form.product_id = null;
                form.stockage_id = null;
              } else {
                form.pharmacy_product_id = null;
                form.pharmacy_stockage_id = null;
              }
            }"
            :disabled="dialogMode === 'view'"
          />
        ...
        <div>
          <label class="tw-block tw-text-sm tw-font-semibold tw-text-gray-700 tw-mb-2">
            <i class="pi pi-calendar tw-mr-2 tw-text-red-600"></i>Expiration Date
          </label>
          <Calendar 
        ...
        <div class="tw-bg-orange-50 tw-border tw-border-orange-200 tw-rounded-xl tw-p-4">
          <div class="tw-flex tw-gap-3">
            ...
            <div>
              ...
              <p class="tw-text-sm tw-text-orange-700">Please provide a reason for cancelling this reservation.</p>
            </div>
          </div>
        </div>
