...
const openCaissePayment = async (fiche) => {
  try {
    const auth = useAuthStore()
    // Ensure auth user is initialized (will return cached user when available)
    const user = await auth.initializeAuth()
    const authId = user?.id ?? auth.user?.id ?? null
    ...
    // Fallback to navigate without cashier_id; server fallback exists
    router.push({
...
const openCreateModal = () => {
    ...
    console.log('Modal state:', { showModal: showModal.value, modalMode: modalMode.value })
}

const openItemsPage = (fiche) => {
    router.push(`/emergency/fiche-navette/${fiche.id}/items`)
}
    ...
    <Card class="tw-bg-white tw-rounded-2xl tw-shadow-lg tw-border tw-border-gray-200">
      <template #content>
        ...
        >
          ...
          <Column field="remaining_amount" header="Montant Restant" sortable>
            <template #body="{ data }">
              <div class="tw-text-right">
