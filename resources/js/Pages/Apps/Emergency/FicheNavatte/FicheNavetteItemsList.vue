...
const groupedItems = computed(() => {
  ...
  items.value.forEach(item => {
    ...
    if (item.package_id) {
      key = `package_${item.package_id}_${item.id}`
    } else if (item.prestation_id) {
      key = `prestation_${item.prestation_id}_${item.id}`
    } else {
      key = `item_${item.id}`
    }

    const groupType = item.is_nursing_consumption ? 'nursing' : (item.package_id ? 'package' : 'prestation')
    const baseName = item.package_id
      ? item.package?.name
      : item.prestation?.name
    const nursingName = item.is_nursing_consumption
      ? (item.nursing_consumptions?.length
          ? `Nursing Care (${item.nursing_consumptions.length} product${item.nursing_consumptions.length > 1 ? 's' : ''})`
          : 'Nursing Care')
      : undefined
    ...
    if (!groups[key]) {
      groups[key] = {
        ...
        name: nursingName || baseName || (groupType === 'package' ? 'Package' : groupType === 'nursing' ? 'Nursing Care' : 'Prestation'),
        doctor_id: item.doctor_id,
        doctor_name: item.doctor?.name,
        items: [],
        total_price: 0,
        parent_item_id: item.id,
        is_nursing: !!item.is_nursing_consumption,
        nursing_product_count: 0,
        nursing_total_price: 0,
      }
    }
    ...
    if (item.is_nursing_consumption && Array.isArray(item.nursing_consumptions)) {
      const nursingTotal = item.nursing_consumptions.reduce((sum, consumption) => {
        const totalPrice = consumption.total_price ?? (consumption.unit_price != null ? Number(consumption.unit_price) * Number(consumption.quantity ?? 0) : 0)
        return sum + (Number(totalPrice) || 0)
      }, 0)

      groups[key].nursing_product_count += item.nursing_consumptions.length
      groups[key].nursing_total_price += nursingTotal
    }
  })
...
const loadConventionCompanies = async () => {
  // Both patient_id AND fiche ID are required for the backend validation
  if (!fiche.value?.patient_id) {
    console.log('Skipping convention companies load: no patient ID')
    return
  }
  
  if (!fiche.value?.id) {
    console.log('Skipping convention companies load: no fiche navette ID')
    return
  }
  
  try {
    loadingConventions.value = true
    console.log('Loading convention companies for patient:', fiche.value.patient_id, 'fiche:', fiche.value.id)
    ...
    if (result.success) {
      ...
      console.log('Loaded convention companies:', conventionCompanies.value.length, 'organismes')
    }
  } catch (error) {
...
<template>
  <div class="fiche-navette-details">
    ...
    <Dialog
      v-model="showConventionDetailsModal"
