    ...
    <div class="tw-max-w-[1600px] tw-mx-auto">
        ...
        <div class="tw-bg-gradient-to-r tw-from-indigo-600 tw-via-purple-600 tw-to-indigo-700 tw-p-8">
          <div class="tw-flex tw-flex-col md:tw-flex-row md:tw-justify-between md:tw-items-center tw-gap-4">
            <div class="tw-flex tw-items-center tw-gap-4">
              ...
              <div>
                <h1 class="tw-text-4xl tw-font-bold tw-text-white tw-tracking-tight">Emergency Doctor Planning</h1>
                <p class="tw-text-indigo-100 tw-mt-2 tw-text-sm">Manage 24/7 doctor schedules and emergency shifts</p>
              </div>
            </div>
            <Button 
      ...
      <div v-else class="tw-bg-white tw-rounded-3xl tw-shadow-xl tw-border tw-border-indigo-100 tw-overflow-hidden">
        <DataTable 
          ...
          :global-filter-fields="['doctor.name', 'doctor.nom', 'doctor.prenom', 'service.name', 'service.nom', 'shift_type']"
        >
          <template #header>
          ...
          <Column field="doctor" header="Doctor" sortable>
            <template #body="slotProps">
              <div class="tw-flex tw-items-center tw-gap-3">
                <Avatar 
                  :label="getPlanningDoctorName(slotProps.data).charAt(0)"
                  class="tw-bg-gradient-to-br tw-from-purple-500 tw-to-indigo-500 tw-text-white"
                  size="small"
                />
                <span class="tw-font-medium tw-text-gray-800">{{ getPlanningDoctorName(slotProps.data) }}</span>
              </div>
            </template>
          </Column>
...
import DoctorEmergencyPlanningModal from './DoctorEmergencyPlanningModal.vue'
...
const getPlanningDoctorName = (planning) => {
  if (!planning) {
    return 'Unassigned'
  }
  
  // Use resource fields first (more reliable)
  if (planning.doctor_display_name) {
    return planning.doctor_display_name
  }

  // Fallback to nested doctor object from resource
  if (planning.doctor?.name) {
    return planning.doctor.name
  }

  if (planning.doctor?.display_name) {
    return planning.doctor.display_name
  }

  if (planning.doctor?.full_name) {
    return planning.doctor.full_name
  }

  // Legacy fallbacks
  if (planning.doctor_name) {
    return planning.doctor_name
  }

  if (planning.doctor_full_name) {
    return planning.doctor_full_name
  }

  if (planning.doctor_nom || planning.doctor_prenom) {
    const parts = [planning.doctor_nom, planning.doctor_prenom].filter(Boolean)
    if (parts.length) {
      return parts.join(' ').replace(/\s+/g, ' ').trim()
    }
  }

  return getDoctorDisplayName(planning.doctor)
}
  ...
  events: plannings.value.flatMap(planning => {
    ...
    if (isOvernight) {
      ...
      events.push({
        ...
        title: `${getPlanningDoctorName(planning)} (${planning.shift_type}) - Night Part 1`,
      ...
      events.push({
        ...
        title: `${getPlanningDoctorName(planning)} (${planning.shift_type}) - Night Part 2`,
      ...
      events.push({
        id: planning.id,
        title: `${getPlanningDoctorName(planning)} (${planning.shift_type})`,
        start: planning.planning_date + 'T' + startTime,
...
const loadPlannings = async () => {
  ...
  try {
    const response = await axios.get('/api/doctor-emergency-planning', { params })
...
const getMonthlyOverview = async () => {
  try {
    ...
    const response = await axios.get('/api/doctor-emergency-planning/overview/monthly', { params })
...
const loadDayOverview = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/doctor-emergency-planning/day-overview', { params })
...
const deletePlanning = (planning) => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await axios.delete(`/api/doctor-emergency-planning/${planning.id}`)
        toast.add({
          severity: 'success',
          summary: 'Success',
...
const copyPlanning = async () => {
  try {
    loading.value = true
    const response = await axios.post('/api/doctor-emergency-planning/copy-planning', copyPlanningForm.value)
...
const printMonthlyReport = async () => {
  try {
    ...
    const response = await axios.get('/api/doctor-emergency-planning/print-report', { params })
...
const generatePrintHTML = (reportData) => {
  ...
  let html = `
    ...
    <body>
      <div class="header">
        <h1>Emergency Doctor Planning Report</h1>
        <h2>${month_name}</h2>
      ...
      <div class="stats">
        ...
        <div class="stat-card">
          <h3>${statistics.total_doctors}</h3>
          <p>Total Doctors</p>
        </div>
        <div class="stat-card">
    ...
    shifts.forEach(shift => {
      html += `
        <div class="shift">
          ...
          <div>${shift.doctor_display_name || 'Unknown Doctor'}</div>
