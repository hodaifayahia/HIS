    ...
    <form @submit.prevent="savePlanning" class="tw-space-y-6">
      ...
      <div class="tw-bg-gradient-to-r tw-from-blue-50 tw-to-indigo-50 tw-rounded-xl tw-p-6 tw-border tw-border-blue-100 tw-shadow-sm">
        <div class="tw-flex tw-items-center tw-gap-3 tw-mb-5">
          ...
          <div>
            ...
            <p class="tw-text-sm tw-text-gray-600 tw-mb-0">Select doctor and service details</p>
          </div>
        </div>
              ...
              <template #option="slotProps">
                <div class="tw-flex tw-items-center tw-gap-2 tw-py-1">
                  <div class="tw-w-8 tw-h-8 tw-bg-gradient-to-br tw-from-blue-400 tw-to-blue-600 tw-rounded-full tw-flex tw-items-center tw-justify-center tw-text-white tw-font-semibold tw-text-sm">
                    {{ slotProps.option.name ? slotProps.option.name.charAt(0).toUpperCase() : 'D' }}
                  </div>
              ...
              <template #value="slotProps">
                <div v-if="slotProps.value" class="tw-flex tw-items-center tw-gap-2">
                  <div class="tw-w-7 tw-h-7 tw-bg-gradient-to-br tw-from-blue-400 tw-to-blue-600 tw-rounded-full tw-flex tw-items-center tw-justify-center tw-text-white tw-font-semibold tw-text-xs">
                    {{ doctors.find(d => d.id === slotProps.value)?.name?.charAt(0).toUpperCase() || 'D' }}
                  </div>
                  <span class="tw-font-medium">{{ doctors.find(d => d.id === slotProps.value)?.name }}</span>
                </div>
                <span v-else class="tw-text-gray-500">{{ slotProps.placeholder }}</span>
              </template>
            </Dropdown>
            <small v-if="errors.doctor_id" class="p-error tw-flex tw-items-center tw-gap-1 tw-mt-1">
      ...
      <div class="tw-bg-gradient-to-r tw-from-purple-50 tw-to-pink-50 tw-rounded-xl tw-p-6 tw-border tw-border-purple-100 tw-shadow-sm">
        ...
        <div class="tw-space-y-4">
          ...
          <div v-if="form.doctor_id && form.planning_date" class="tw-bg-gradient-to-r tw-from-emerald-50 tw-to-teal-50 tw-border-2 tw-border-emerald-200 tw-rounded-xl tw-p-4 tw-shadow-sm">
            ...
            <div class="tw-flex tw-items-start tw-justify-between">
              <div class="tw-flex tw-items-start tw-gap-3">
                <div class="tw-w-10 tw-h-10 tw-bg-gradient-to-br tw-from-red-400 tw-to-red-600 tw-rounded-full tw-flex tw-items-center tw-justify-center tw-text-white tw-font-bold tw-text-sm tw-flex-shrink-0">
                  {{ (conflict.doctor_display_name || conflict.doctor?.name || conflict.doctor?.display_name || 'Unknown').charAt(0).toUpperCase() }}
                </div>
                <div>
                  <div class="tw-font-bold tw-text-red-900 tw-text-base tw-mb-1">
                    {{ conflict.doctor_display_name || conflict.doctor?.name || conflict.doctor?.display_name || 'Unknown Doctor' }}
                  </div>
...
const populateForm = () => {
  if (props.planning) {
    form.value = {
      doctor_id: props.planning.doctor_id,
...
const checkConflicts = async () => {
  if (!form.value.doctor_id || !form.value.planning_date || 
  ...
  try {
    ...
    const response = await axios.post('/api/doctor-emergency-planning/check-conflicts', params)
...
const getSuggestedStartTime = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/doctor-emergency-planning/next-available-time', { params })
...
const savePlanning = async () => {
  ...
  try {
    ...
    let response
    if (isEditing.value) {
      response = await axios.put(`/api/doctor-emergency-planning/${props.planning.id}`, payload)
    } else {
      response = await axios.post('/api/doctor-emergency-planning', payload)
    }
