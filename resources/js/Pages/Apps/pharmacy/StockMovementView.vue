<template>
  <div class="tw-min-h-screen tw-bg-gradient-to-br tw-from-blue-50 tw-via-indigo-50 tw-to-purple-50 tw-p-6">
    ...
    <div class="tw-fixed tw-bottom-8 tw-right-8 tw-flex tw-flex-col tw-gap-4 tw-z-50">
      <Button
        ...
        <div class="tw-flex tw-justify-between tw-items-center">
          ...
          <div class="tw-flex tw-space-x-3">
            ...
            <Button
              ...
              @click="openAddProductDialog"
              icon="pi pi-plus"
              class="tw-bg-green-600 tw-rounded-xl hover:tw-bg-green-700 tw-px-4 tw-py-2 tw-text-nowrap"
              v-tooltip.top="'Add Product'"
            >
              Add Product
            </Button>
  ...
  <ProductSelectionDialog
    ...
    :service-id="movement?.providing_service_id"
...
export default {
  ...
  setup(props) {
    ...
    const loadMovement = async () => {
      try {
        loading.value = true
        const response = await axios.get(`/api/pharmacy/stock-movements/${props.movementId}`)
    ...
    const approveSelectedItems = async () => {
      try {
        ...
        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/approve`, {
    ...
    const rejectSelectedItems = async () => {
      try {
        ...
        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/reject`, {
  ...
  try {
    ...
    const confirmDeliveryGood = async () => {
      try {
        ...
        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/confirm-delivery`, {
          status: 'good',
    ...
    const confirmProductGood = async (product) => {
      try {
        ...
        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/confirm-product`, {
          item_id: product.id,
          status: 'good',
    ...
    const finalizeConfirmation = async () => {
      try {
        ...
        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/finalize-confirmation`, {
    ...
    const validateQuantities = async () => {
      try {
        ...
        // Call validation API
        const response = await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/validate-quantities`, {
    ...
    const processValidation = async () => {
      try {
        ...
        const response = await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/process-validation`, {
    ...
    const loadAvailableProducts = async () => {
      try {
        ...
        const response = await axios.get(`/api/pharmacy/products/available/${movement.value.providing_service_id}`)
    ...
    const addProduct = async () => {
      ...
      try {
        ...
        const response = await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/items`, {
    ...
    const getSelectedProductUnit = () => {
      if (!selectedProduct.value) return 'units'
      return getProductUnit(selectedProduct.value)
    }

    // Watch for product selection changes
    watch(() => newItem.value.product_id, (newProductId) => {
      if (newProductId) {
        selectedProduct.value = availableProducts.value.find(p => p.id === newProductId)
      } else {
        selectedProduct.value = null
      }
    })
