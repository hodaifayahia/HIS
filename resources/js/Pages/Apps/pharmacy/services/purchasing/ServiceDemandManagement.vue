        ...
        <div class="tw-mb-6 tw-grid tw-grid-cols-1 md:tw-grid-cols-4 tw-gap-4">
          ...
          <div class="tw-flex tw-flex-col">
            ...
            <Dropdown
              ...
              class="tw-w-full"
              :disabled="!!routeServiceId"
              @change="loadDemands"
            />
            <small v-if="routeServiceId" class="tw-text-gray-500 tw-mt-1">
...
// Get service ID from route params if available
const routeServiceId = computed(() => {
  return route.params.id || props.serviceId;
});
...
// Lifecycle
onMounted(() => {
  // If serviceId is provided from route or props, set it as the filter
  if (routeServiceId.value) {
    serviceFilter.value = parseInt(routeServiceId.value);
    // Also set it in newDemand for pre-selection when creating new demands
    newDemand.value.service_id = parseInt(routeServiceId.value);
  }
  loadInitialData();
});
...
const loadServices = async () => {
  ...
  try {
    ...
    // If serviceId is from route or props, find and set the service name
    if (routeServiceId.value) {
      const selectedService = allServices.value.find(s => s.id == routeServiceId.value);
      if (selectedService) {
...
const createDemandAndRedirect = async () => {
  ...
  try {
    ...
    const selectedService = services.value.find(s => s.id === newDemand.value.service_id) || 
                           allServices.value.find(s => s.id === newDemand.value.service_id);
    
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: `Demand created successfully for ${selectedService?.name || 'Service'}!`,
      life: 3000
    });
    
    closeCreateDialog();
    
    // Navigate to edit mode with the new demand and pass service info
    router.push({
      path: `/pharmacy/service-demands/edit/${newDemandData.id}`,
      query: { 
        mode: 'edit', 
        skipBasicInfo: 'true',
        service_id: newDemand.value.service_id,
...
const resetNewDemand = () => {
  newDemand.value = {
    service_id: routeServiceId.value ? parseInt(routeServiceId.value) : null,
    expected_date: null,
    notes: ''
  };
};
...
const viewDemand = (demand) => {
  ...
  router.push({
    path: `/pharmacy/service-demands/view/${demand.id}`,
    query: { mode: 'view' }
  });
};
...
const clearFilters = () => {
  ...
  // Don't clear serviceFilter if serviceId is from route
  if (!routeServiceId.value) {
