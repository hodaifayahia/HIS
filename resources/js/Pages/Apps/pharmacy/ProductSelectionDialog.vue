...
export default {
  ...
  setup(props, { emit }) {
    ...
    const loadInventory = async () => {
      ...
      try {
        loading.value = true
        const response = await axios.get(`/api/pharmacy/stockages?service_id=${props.serviceId}`);
    ...
    const saveSelection = async () => {
      try {
        ...
        if (!selectedItems.value || Object.keys(selectedItems.value).length === 0) {
          toast.add({
            severity: 'warn',
            summary: 'Warning',
            detail: 'Please select at least one inventory item',
            life: 3000
          })
          return
        }
        ...
        const processedSelections = Object.entries(selectedItems.value)
          .filter(([id, item]) => item && item.quantity > 0) // Filter out invalid items
          .map(([id, item]) => {
            const inventoryItem = inventory.value.find(i => i.id === parseInt(id))
            const boiteDe = inventoryItem?.product?.boite_de || props.selectedItem?.product?.boite_de || 1
            
            const finalQuantity = item.byBox ? (item.quantity * boiteDe) : item.quantity
        ...
        // Validate processed selections
        if (processedSelections.length === 0) {
          toast.add({
            severity: 'warn',
            summary: 'Warning',
            detail: 'Please select valid quantities for inventory items',
            life: 3000
          })
          return
        }
        ...
        for (const selection of processedSelections) {
          if (!selection.inventory_id || !selection.quantity || selection.quantity <= 0) {
            toast.add({
              severity: 'error',
              summary: 'Validation Error',
              detail: 'All selected items must have valid inventory ID and quantity greater than 0',
              life: 3000
            })
            return
          }
        ...
        console.log('Sending selection data:', selectionData) // Debug log

        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/select-inventory`, selectionData)
