  ...
  setup(props) {
    ...
    const loadAvailableProducts = async () => {
      try {
        ...
        const firstPageResponse = await axios.get(`/api/pharmacy/products?service_id=${movement.value?.providing_service_id}&per_page=500&nocache=${Date.now()}`)
        ...
        if (!meta.last_page || meta.last_page <= 1) {
          ...
          for (let page = 2; page <= totalPages; page++) {
            remainingPagePromises.push(
              axios.get(`/api/pharmacy/products?service_id=${movement.value?.providing_service_id}&per_page=500&page=${page}&nocache=${Date.now()}`)
                .catch(error => {
        ...
        const suggestionsResponse = await axios.get(`/api/pharmacy/stock-movements/suggestions?providing_service_id=${movement.value?.providing_service_id}`)
    ...
    const updateItemQuantity = async (item) => {
      try {
        ...
        await axios.put(`/api/pharmacy/stock-movements/${props.movementId}/items/${item.id}`, {
    ...
    const removeItem = async (item) => {
      ...
      try {
        await axios.delete(`/api/pharmacy/stock-movements/${props.movementId}/items/${item.id}`)
    ...
    const sendRequest = async () => {
      ...
      try {
        sending.value = true
        await axios.post(`/api/pharmacy/stock-movements/${props.movementId}/send`)
