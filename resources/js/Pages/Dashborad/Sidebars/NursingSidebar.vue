...
const appDetails = {
    name: 'nursing Desk',
    icon: '/storage/nursing-icon.png', // Example icon, or use 'fas fa-clipboard-list'
    color: '#', // Example color
    backRoute: '/home'
};
...
<template>
    <BaseSidebar :user="authStore.user" :app-name="appDetails.name" :app-icon="appDetails.icon"
        ...
        <template #navigation>
            <li class="nav-item">
                <router-link to="/nursing/dashboard" active-class="active" class="nav-link">
            ...
            <li class="nav-item has-treeview" :class="{ 'menu-is-opening menu-open': isPatientManagementOpen }">
                <router-link to="/nursing/product" active-class="active" class="nav-link">
                    <i class="far fa-circle nav-icon"></i>
                    <p>Nursing Product</p>
                </router-link>
            </li>
