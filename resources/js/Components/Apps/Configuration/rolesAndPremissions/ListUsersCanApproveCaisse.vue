...
const getUsers = async (page = 1) => {
    ...
    try {
        ...
        const res = await axios.get('/api/user-caisse-approval', { params });
        const data = res.data;
...
const togglePermission = async (user) => {
    try {
        if (user.can_approve_caisse) {
            // Remove permission
            await axios.delete(`/api/user-caisse-approval/${user.id}`);
            user.can_approve_caisse = false;
            // ✅ Updated toast call
            toast.add({severity: 'success', summary: 'Success', detail: `Caisse approval permission removed from ${user.name}`, life: 3000});
        } else {
            // Grant permission
            const payload = { user_id: user.id };
            await axios.post('/api/user-caisse-approval', payload);
            user.can_approve_caisse = true;
            // ✅ Updated toast call
            toast.add({severity: 'success', summary: 'Success', detail: `Caisse approval permission granted to ${user.name}`, life: 3000});
        }
    } catch (err) {
...
<template>
  <div class="tw-p-6 tw-bg-gray-100 tw-min-h-screen">
    <div class="tw-container tw-mx-auto">
      <div class="tw-mb-6">
        <h1 class="tw-text-3xl tw-font-bold tw-text-gray-800">Users Can Approve Caisse</h1>
        <p class="tw-text-gray-600 tw-mt-2">Manage users who can approve caisse transactions</p>
      </div>
        ...
        <table class="tw-min-w-full tw-table-auto">
          <thead>
            <tr class="tw-bg-gray-100 tw-text-gray-600 tw-uppercase tw-text-sm tw-leading-normal">
              ...
              <th class="tw-py-3 tw-px-6 tw-text-center">Can Approve Caisse</th>
            </tr>
          </thead>
                ...
                <div class="tw-relative tw-inline-block tw-w-10 tw-h-6">
                  <label>
                    <input 
                      ...
                      :checked="Boolean(user.can_approve_caisse)"
                    ...
                    <span 
                      ...
                      :class="{ 'tw-bg-green-500': Boolean(user.can_approve_caisse), 'tw-bg-gray-200': !Boolean(user.can_approve_caisse) }"
                    ></span>
                    <!-- Toggle switch dot -->
                    <span 
                      ...
                      :class="{ 'tw-translate-x-4': Boolean(user.can_approve_caisse) }"
                    ></span>
                  </label>
                </div>
              </td>
