<template>
  <div class="tw-min-h-screen tw-bg-gradient-to-br tw-from-slate-50 tw-via-blue-50/30 tw-to-indigo-50 tw-p-4 lg:tw-p-6">
              ...
              <div class="tw-flex tw-items-center tw-gap-4">
                ...
                <div>
                  <h1 class="tw-text-3xl tw-font-bold tw-text-white tw-flex tw-items-center tw-gap-3">
                    Create New Facture Proforma
                    <Tag value="Draft" severity="warning" icon="pi pi-pencil" class="tw-font-semibold" />
                  </h1>
                  <p class="tw-text-emerald-100 tw-mt-1">
                    <i class="pi pi-info-circle tw-mr-2"></i>
                    Create a new proforma invoice for purchasing
                  </p>
                </div>
              </div>
                ...
                <div class="tw-flex tw-items-center tw-justify-between">
                  <div>
                    <p class="tw-text-sm tw-font-semibold tw-text-purple-600 tw-flex tw-items-center tw-gap-2">
                      <i class="pi pi-paperclip tw-text-purple-500"></i>
                      Files
                    </p>
                    <p class="tw-text-3xl tw-font-bold tw-text-purple-800 tw-mt-1">{{ formData.attachments.length }}</p>
                  </div>
                  <div class="tw-p-3 tw-bg-purple-100 tw-rounded-xl">
                    <i class="pi pi-paperclip tw-text-2xl tw-text-purple-600"></i>
                  </div>
                </div>
              </div>
                ...
                <div class="tw-flex tw-items-center tw-justify-between">
                  <div>
                    <p class="tw-text-sm tw-font-semibold tw-text-orange-600 tw-flex tw-items-center tw-gap-2">
                      <i class="pi pi-percentage tw-text-orange-500"></i>
                      Progress
                    </p>
                    <p class="tw-text-3xl tw-font-bold tw-text-orange-800 tw-mt-1">{{ formProgress }}%</p>
                  </div>
                  <div class="tw-p-3 tw-bg-orange-100 tw-rounded-xl">
                    <ProgressBar :value="formProgress" :showValue="false" class="tw-w-8 tw-h-8" />
                  </div>
                </div>
              </div>
              ...
              <div class="tw-bg-white tw-rounded-2xl tw-p-4 tw-border tw-border-emerald-100 hover:tw-shadow-lg tw-transition-all tw-duration-300 hover:tw-scale-105">
                <div class="tw-flex tw-items-center tw-justify-between">
                  <div>
                    ...
                    <p class="tw-text-2xl tw-font-bold tw-text-emerald-800 tw-mt-1">{{ formatCurrency(totalAmount) }}</p>
                  </div>
    ...
    <form @submit.prevent="handleSubmit" class="tw-space-y-6">
      ...
      <Card class="tw-border-0 tw-shadow-xl hover:tw-shadow-2xl tw-transition-shadow enhanced-card">
        <template #title>
    ...
    >
      <div class="tw-space-y-4">
        <!-- Reusable Product Selector with Infinite Scroll -->
        ...
        <div v-if="selectedProducts.length > 0" class="tw-bg-gradient-to-r tw-from-teal-50 tw-to-cyan-50 tw-p-4 tw-rounded-lg tw-border tw-border-teal-200">
          ...
          <div class="tw-grid tw-grid-cols-3 tw-gap-4">
            ...
            <div>
              <label class="tw-text-sm tw-font-medium tw-text-gray-700">Default Unit Price (DZD)</label>
              <InputNumber
    ...
    >
      <div class="tw-space-y-6">
        ...
        <div>
          ...
          <div class="tw-flex tw-items-center tw-gap-6 tw-mb-4 tw-p-4 tw-bg-gray-50 tw-rounded-lg tw-border tw-border-gray-200">
            <div class="tw-flex tw-items-center tw-gap-2">
              <RadioButton 
                v-model="newProductForm.is_clinical"
                name="product_type"
                :value="false"
                inputId="stock_product"
              />
              <label for="stock_product" class="tw-text-sm tw-font-medium tw-text-gray-700 tw-cursor-pointer">
                Stock Product
              </label>
              <span class="tw-text-xs tw-text-gray-500">(Inventory/Stock)</span>
            </div>
            <div class="tw-flex tw-items-center tw-gap-2">
              <RadioButton 
                v-model="newProductForm.is_clinical"
                name="product_type"
                :value="true"
                inputId="pharmacy_product"
              />
              <label for="pharmacy_product" class="tw-text-sm tw-font-medium tw-text-gray-700 tw-cursor-pointer">
                Pharmacy Product
              </label>
              <span class="tw-text-xs tw-text-gray-500">(Pharmacy)</span>
            </div>
          </div>
...
// Data
const suppliers = ref([])
const products = ref([]) // All available products for selection
const selectedProducts = ref([]) // Products selected in bulk add modal
...
const fetchProducts = async () => {
  try {
    ...
    // Fetch both stock and pharmacy products
    const [stockResponse, pharmacyResponse] = await Promise.all([
      axios.get('/api/products?per_page=1000'),
      axios.get('/api/pharmacy/products?per_page=1000')
    ])
    
    const stockProducts = (stockResponse.data.data || []).map(p => ({
      ...p,
      source: 'stock',
      is_clinical: false
    }))
    
    const pharmacyProducts = (pharmacyResponse.data.data || []).map(p => ({
      ...p,
      source: 'pharmacy',
      is_clinical: true
    }))
    
    products.value = [...stockProducts, ...pharmacyProducts]
  } catch (err) {
...
const handleProductSelectionChange = (products) => {
  // Optional: Add any custom logic when product selection changes
...
const onProductChange = (item, index) => {
  // Product unit is now managed by the component
...
const showProductInfo = (productId) => {
  // Product info display - fetch if needed
  toast.add({
    severity: 'info',
...
// Bulk Add - handled by component now

const addBulkProducts = () => {
  selectedProducts.value.forEach(product => {
    // Add product to main products array if not already there
    if (!products.value.find(p => p.id === product.id)) {
      products.value.push({
        ...
        unit: product.unit || product.forme || 'pieces',
        source: product.source,
        is_clinical: product.is_clinical
      })
    }
    
    // Add to form products using default values from product or global defaults
    const defaultQuantity = product.default_quantity || bulkDefaults.quantity || 1
    const defaultUnit = product.default_unit || bulkDefaults.unit || product.unit || product.forme || 'Unit'
    const defaultPrice = product.default_price || bulkDefaults.unit_price || 0
    ...
    formData.products.push({
      ...
      total_price: defaultQuantity * defaultPrice,
      notes: ''
    })
  })
...
const handleBulkAdd = (bulkData) => {
  // bulkData now contains { products: [...], defaults: {...} }
  selectedProducts.value = bulkData.products
  
  // Update bulkDefaults with the global defaults from the component
  Object.assign(bulkDefaults, bulkData.defaults)
...
const createProduct = async () => {
  try {
    ...
    const response = await axios.post('/api/purchasing/products', {
    ...
    const errorMsg = err.response?.data?.message || err.response?.data?.error || 'Failed to create product'
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: errorMsg,
...
const handleSubmit = async (isDraft = false) => {
  ...
  try {
    ...
    formDataPayload.append('status', isDraft ? 'draft' : 'sent')
