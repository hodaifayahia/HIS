...
export default {
  ...
  methods: {
    ...
    async fetchStockageLocations(stockageId) {
      ...
      try {
        const response = await axios.get(`/api/stockages/${stockageId}/tools`);
    ...
    async updateLocationSuggestions() {
      ...
      if (locationOptions.length > 0 && !this.formData.location) {
        this.formData.location = locationOptions[0].value;
      }
    },
    ...
    closeAddLocationModal() {
      ...
      if (this.locationOptions.length > 0) {
        const firstOption = this.locationOptions.find(opt => opt.value !== 'new_location');
        this.formData.location = firstOption ? firstOption.value : '';
      } else {
    ...
    async createLocation() {
      ...
      try {
        ...
        const response = await axios.post(`/api/stockages/${this.newLocation.stockage_id}/tools`, locationData);
        if (response.data.success) {
          const newLoc = {
    ...
    async addProductToStock() {
      ...
      this.isSubmitting = true;
        ...
        const response = await axios.post('/api/inventory', stockData);
        if (response.data.success) {
